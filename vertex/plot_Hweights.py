...
heatmapping(h_z, np.round(np.min(h_z))+1.999, np.round(np.max(h_z))-1.9999, 2, args.width, args.height, fname=fname)
...
heatmapping(h_norm,args.minimum,args.maximum+0.0001,0.25,args.width,args.height,title="Cross-sectional NMF",fname=fname)    
...
heatmapping(h,raw_minn,raw_maxx,space,args.width,args.height,title="Cross-sectional NMF",fname=fname)
